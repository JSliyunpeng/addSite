...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      getSiteList() {
        this.$axios.get("/h5/youlaYi/list", {  //获取列表
          params: {
            condtion: {
      ...
      CloseIndexList(value) {  //获取区域列表
        ...
        if (value) {
          this.$axios.get('/gunsApi/region/list', {params: {pid: value.item.id}}).then(result => {
      ...
      initGeolocation() {
        let sessionStartLng = sessionStorage.getItem('startLng')
      ...
      initMyMap() {
        ...
        AMap => {
          ...
          AMap.event.addListener(geolocation, 'complete', (data) => {// 具体的定位信息
            ...
            sessionStorage.setItem('startLng', data.position.lng)
            sessionStorage.setItem('startLat', data.position.lat)
          ...
          AMap.event.addListener(geolocation, 'error', (data) => {
            sessionStorage.setItem('startLng', that.startLngLat.lng)
            sessionStorage.setItem('startLat', that.startLngLat.lat)
            that.getSiteList()
          })
        },
        e => {
