    ...
    <transition name="fade">
      <div class="details" v-show="detialContent.name && $store.state.mapMiniCard">
        ...
        <router-link class="site" :to="{path:'/details',query:{id:detialContent.id,distance:detialContent.distance}}">
          <img class="site_img" v-if="detialContent.imagesF" :src="'http://106.12.163.166:8000'+detialContent.imagesF" alt>
    ...
    created() {
      ...
      .get("/h5/youlaYi/list", {
        params: {
          condtion: {
            sort: 0,
            coordinateX: sessionStorage.getItem('startLng'),
            coordinateY: sessionStorage.getItem('startLat'),
          },
          youlaitType: 0
        }
      })
