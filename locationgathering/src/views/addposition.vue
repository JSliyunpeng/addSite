      ...
      <div v-for="(title,key) in imgList" v-if="title && title.length > 0">
        ...
        <div class="img_box clearfix">
          <div class="img_box_inner" v-for="(item,index) in title">
            ...
            <img :key="index" :src="'http://106.12.163.166:8000'+item.url">
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        this.$axios.get('/h5/youlaYi/view', {params: {id: this.prams.mid}}).then(res => {
          let result = res.data
      ...
      submit() {
        ...
        console.log(params)
        this.$axios.post("/h5/youlaYi/update", params).then(result => {
          if (result) {
            this.showPopup()
          }
        }).catch((err) => {
    ...
    created() {
      this.imgList = {}
      this.prams.mid = this.$route.query.id
      if (this.prams.mid) {
        this.init()
      } else {
