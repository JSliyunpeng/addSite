  ...
  <div class="add_merchant">
    <div v-show="!showYoulaihui">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="邀请码" prop="createBy">
          <el-input v-model="ruleForm.createBy" placeholder="请填写邀请码"></el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      var validateCreated = (rule, value, callback) => {
        // const reg = /^[\d_a-zA-Z]{4,4}$/ //必须是字母和数字混合
        const reg = /^\d{4,9}$/
        console.log(value)
        if (!reg.test(value)) {
          callback(new Error('请填写4-9位数字的邀请码'))
        } else {
          callback()
        }
      };
      ...
      return {
        ...
        rules: {
          ...
          phone: [
            ...
            // {max: 11, min: 11, message: '请输入正确的手机号', trigger: 'change'},
            {validator: validatePhone, trigger: 'blur'}
          ],
          createBy: [
            // {required: true,min: 4, max: 6, message: '2请填写4-6位字母数字的邀请码', trigger: 'blur'},
            {required: true, min: 4, max: 9, message: '请填写4-9位数字的邀请码', trigger: 'change'},
            {validator: validateCreated, trigger: 'blur'}
          ]
          // verificationCode: [
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            form.createBy = window.localStorage.getItem('bankId')
            form.createBy = 1000
            this.$axios.post('http://106.12.163.166:8000/register', form).then(res => {
              if (res.data.code == 200) {
                ...
                window.localStorage.setItem('account', JSON.stringify(this.ruleForm))
              } else {
                this.$createDialog({
                  type: 'alert',
                  title: '',
