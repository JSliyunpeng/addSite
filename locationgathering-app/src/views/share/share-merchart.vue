  ...
  <div class="add_merchant">
    <div v-show="!showYoulaihui">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <!--          </button>-->
        <!--        </div>-->
<!--        <el-button @click="toDetial" class="jixu" style="margin-left: -30%" type="primary">&ndash;&gt;-->
<!--                        继续填写营地信息-->
<!--                      </el-button>-->
  ...
  export default {
    ...
    methods: {
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            // delete form.mid
            form.createBy = (JSON.parse(window.localStorage.getItem('userView'))).bankId
            this.$axios.post('http://106.12.163.166:8000/h5/addYoulaiBaseInfo', form).then(res => {
              let result = res.data
              console.log(result)
              if (result.code == 200) {
      ...
      initPickerAddress() { // 选择城市初始化
        this.$axios.get('http://106.12.163.166:8000/h5/getListByCube').then(res => {
          // let result = res.data
      ...
      createCode() { // 创建编号
        if (this.ruleForm.youlaitType != '' && this.cityId != '') {
          this.$axios.get('http://106.12.163.166:8000/h5/createCode', {
            params: {
