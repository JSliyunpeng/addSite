...
<script>
  ...
  export default {
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$axios.post('http://106.12.163.166:8000/h5/youlaYi/update', form).then(res => {
              let result = res.data
              console.log(result)
      ...
      init() { // 初始化方法
        if (this.mid) {
          this.$axios.get('http://106.12.163.166:8000/h5/youlaYi/view', {params: {id: this.$route.query.id}}).then(res => {
            let result = res.data
            console.log(result)
